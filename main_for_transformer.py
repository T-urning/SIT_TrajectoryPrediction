...
parser.add_argument('--spatial_interact', default=True, type=bool)
parser.add_argument('--teacher_forcing', default=0.6, type=float)
...
def data_loader(*data_fpaths, batch_size=128, shuffle=True, drop_last=False, train_val_test='train'):
    ...
    for data_path in data_fpaths:
        dataset = NgsimFeederIII(data_path, train_val_test=train_val_test, **args.__dict__)
...
def run_train_val(model, train_data_path, dev_data_path, test_data_path, resume_training=False):
    ...
    #     optimizer, mode='min', factor=0.8, patience=num_batch_train, min_lr=1e-6
...
if __name__ == '__main__':
    ...
    if args.do_test:
        ...
        run_test(model, test_data_path=dev_path, model_path=model_path, congest_scene_only=True)
